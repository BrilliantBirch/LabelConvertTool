...
def readlabels(dataRoot):
    ...
    # 获取所有图片和标注文件
    image_files = [os.path.join(img_dir, f) for f in os.listdir(
        img_dir) if f.endswith('.jpg') or f.endswith('.png')]
    label_files = [f for f in os.listdir(
        label_dir) if f.endswith('.txt') and f != 'classes.txt']

    image_names = set(os.path.splitext(os.path.basename(i))
                      [0] for i in image_files)
...
def splitData(images, output):
    ...
    # 分割数据集
    train_images, rest_images = train_test_split(
        images, test_size=num_val + num_test)
...
def parsers():
    ...
    parser.add_argument(
        '--data', type=str, default=r'E:\OCRSceneImage\TrainData\YOLO', help='yolo数据根目录')
    parser.add_argument(
        '--output', type=str, default=r'E:\OCRSceneImage\TrainData\YOLO\split', help='数据集分割导出目录')
    parser.add_argument('--classesTxt', type=str,
                        default=r'E:\OCRSceneImage\TrainData\YOLO\labels\classes.txt', help='classes.txt路径')
